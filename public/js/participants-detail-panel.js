document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    if (res.ok && p) {
        ...
        <div style="margin-bottom: var(--spacing-lg);">
          ...
          <table style="width: 100%; text-align: left; border-collapse: collapse; margin-bottom: var(--spacing-md);">
            <tbody>
              ...
              <tr><th style="padding: 16px 0; border-bottom: 1px solid var(--colors-hairline);">Event ID</th><td style="padding: 16px 0; text-align: right; border-bottom: 1px solid var(--colors-hairline);">${p.event_id}</td></tr>
            </tbody>
          </table>
        </div>
      ...
      container.innerHTML = '<p class="typography-body-md" style="color: var(--colors-danger);">Participant not found.</p>';
    }
  } catch(err) {
    console.error(err);
    container.innerHTML = '<p class="typography-body-md" style="color: var(--colors-danger);">Failed to load participant details.</p>';
  }
});
